...
class QueryDslGroupingTest {
    ...
    void ageGroupTest () {
        ...
        List<Tuple> result = factory
                ...
                .having(idol.count().gt(5)) // 카운트가 5이상인것
                .fetch();
        //then
    ...
    @Test
    @DisplayName("그룹별로 아이돌의 그룹명과 평균 나이가 20세와 25세 사이인 그룹만 조회한다.")
    ...
    @Test
    @DisplayName("그룹별로 아이돌의 그룹명과 평균 나이가 20세와 25세 사이인 그룹만 조회한다.(결과 DTO 처리)")
    void groupAverageAgeDtoTest () {
        ...
        List<GroupAverageAgeDto> result = factory
                .select(
                        Projections.constructor( // 첫번째로는 DTO 넣어주고, 생성자 써주면 됨
